...
?>
  ...
  /**
    ...
    public function related_opts($model){
        ...
        $options='';
       $products=GxHtml::listDataEx(<?php echo $relatedModelClass ?>::model()->findAllAttributes(null, true));
  ...
  $this->render('admin_list', array(
   ...
   //prepare dataproviders  for Category and Product models
    ...
    private function prepare_dataproviders()
    {
        ...
        $cat_dataProvider = new CActiveDataProvider('<?php echo $this->modelClass; ?>');

        $prod_criteria = new CDbCriteria;
        $prod_criteria->with = array('<?php echo $relatedRelationName ?>' => array(
            'on' => '<?php echo $relatedRelationName ?>.id=:cat_id',
            'together' => true,
            'joinType' => 'INNER JOIN',
        ...
        //for search
        $model = new <?php echo $relatedModelClass ?>('search');
        $model->unsetAttributes();
        if (isset($_GET['<?php echo $relatedModelClass ?>'])) { //if searching
            $model->setAttributes($_GET['<?php echo $relatedModelClass ?>']);
            $search_criteria = $model->searchCriteria();
            $prod_criteria->mergeWith($search_criteria);
        }

        $prod_dataProvider = new CActiveDataProvider('<?php echo $relatedModelClass ?>', array(
   ...
   public function actionRename(){
     ...
     public function actionCreateRoot()
    {
        if (isset($_POST['<?php echo $this->modelClass; ?>'])) {
            ...
            $new_root->setAttributes($_POST['<?php echo $this->modelClass; ?>']);
            $relatedData = array(
    ...
    public function actionCreate()
    {
        if (isset($_POST['<?php echo $this->modelClass; ?>'])) {
            ...
            if ($model->appendToWithRelated(array('<?php echo $relationName; ?>' => $_POST['<?php echo $this->modelClass; ?>']['<?php echo $relationName; ?>']), $parent)) {
                echo json_encode(array('success' => true,
                                      'id' => $model->primaryKey
                                 )
                );
    ...
    public function actionUpdate()
    {
        ...
        if (isset($_POST['<?php echo $this->modelClass; ?>'])) {
            ...
            if ($model->saveNodeWithRelated(array('<?php echo $relationName; ?>' => $_POST['<?php echo $this->modelClass; ?>']['<?php echo $relationName; ?>']), false)) {
                echo json_encode(array('success' => true,
                                      'update_id' => $_POST['update_id'],
                                      'updated_name' => $model->name,
                                      'updated_img' => $model-><?php echo  $this->class2var($this->modelClass);?>ImgBehavior->getFileUrl('thumb')));
            }
...
//maps all move and copy actions on jstree to  serverside database operations.
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='<?php echo  $this->class2var($this->modelClass);?>-form')
		{
